...
3. components are created in accordance with [Atomic Design Methodology ↗](https://atomicdesign.bradfrost.com/chapter-2/)
4. package already contains some basic components and can be extended when developing an app
...
const Sample = ({ prop1, prop2 }: SampleProps) => (
...
Displays content within a desired ratio. Uses [React Aspect Ratio primitive from radix-ui ↗](https://www.radix-ui.com/primitives/docs/components/aspect-ratio).
Usage:
...
interface SampleProps {
  className: string;
  variant: string<defined variants>;
  size: string<defined sizes>;
  asChild: boolean;
};
...
```tsx {1, 10}
import { Code } from '@natu/ui';

interface SampleProps {
  className?: string;
  code?: string;
  language?: string<defined languages>;
}
...
[Dropdown Menu primitive from radix-ui ↗](https://www.radix-ui.com/primitives/docs/components/dropdown-menu)
...
Ready-to-use popover component. Based on [Popover primitive from radix-ui ↗](https://www.radix-ui.com/primitives/docs/components/popover)
(go there for all implementation & usage details). Usage:
...
It's used mainly in [storyblok-richtext package](https://naturaily-starter-docs.vercel.app/packages/storyblok/storyblok-richtext)
...
[Scroll Area primitive from radix-ui ↗](https://www.radix-ui.com/primitives/docs/components/scroll-area)
(go there for all implementation & usage details). Usage:
...
Ready-to-use tooltip component. Uses [Tooltip primitive from radix-ui ↗](https://www.radix-ui.com/primitives/docs/components/tooltip)
(go there for all implementation & usage details). Usage:
...
Uses [Slot utility from radix-ui ↗](https://www.radix-ui.com/primitives/docs/utilities/slot). Usage:
...
interface SampleProps {
  children?: ReactNode;
  variant?: string <defined variants>;
  component?: ElementType <ex. 'h1', 'quote', 'p', 'div'>;
  asChild?: boolean;
}

export const Sample = ({ children, variant, component, asChild}) => (
  <Typography component={tag} variant={variant} asChild={asChild}>
    {children}
  </Typography>
);
```
