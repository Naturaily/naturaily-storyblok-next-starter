...
for more details. Already used in [StoryblokRichtext](/packages/storyblok/storyblok-richtext#storyblokrichtext) component. Resolvers included in the package:
...
The `variant` prop allows you to choose between different styles of rendering the richtext content. Available variants include:

- `default`: The standard styling for richtext content.
- `prose`: A more visually rich styling, suitable for longer texts and articles.

Additionally, you can create your own custom variant by extending the `StoryblokRichtext` component and passing a different `className` or `styles`.
