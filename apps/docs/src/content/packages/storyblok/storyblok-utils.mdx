...
```tsx copy filename="SBContainer.tsx" /DynamicRender/ {2,15}
import type { ElementType } from 'react';
import { BlokItem, SBProps } from '../../../utils/types/SBProps/SBProps';

interface SBContainerProps {
  body?: BlokItem[]; // <- blocks field in Storyblok CMS
}
...
```tsx copy filename="SBTable.tsx" /StoryblokTable/ {11-13,17,20,26,33,39}
import {
  Table,
  TableBody,
  TableCaption,
...
An interface describing the storyblok asset object and an auxiliary function that normalizes the data

_Example:_

```tsx copy filename="SBImage.tsx" /StoryblokAsset/ {4,9,15,21}
...
Depending on what the user selects in the storyblok (`url link, internal link, email`), a different object will be returned by the CMS next to the link field. This function normalizes it.
...
A feature that allows you to add a specific border for a storyblok (`preview mode support`). After clicking on such a component in the CMS, you will jump to it and we will be able to quickly edit it.
...
In storyblok applications, we use a `datasource` as one place for our reusable data. This is often used as eg max-width, spacing in components. This function stores all this data and returns the appropriate styles based on the keys.
...
```ts copy filename="packages/storyblok/src/utils/resolveStoryblokStyles/styles/spacing/Spacing.ts" /36/
export type Spacing = '2' | '4' | '6' | '8' | '10' | '12' | '16' | '20' | '36';
```

3. Now, the objects need to be filled in with the appropriate classes.

```ts filename="packages/storyblok/src/utils/resolveStoryblokStyles/styles/spacing/Spacing.ts" /36/ {10,22}
